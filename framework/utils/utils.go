...
func ItoFloat64(val interface{}) float64{
	f, ok := val.(float64) 
    if ok {
        return f
    } else {
...
// FormatUnits chuyển đổi từ Wei sang số dễ đọc
func FormatUnits(value *big.Int, decimals,round int) string {
	// 10^decimals để làm hệ số chia
	factor := new(big.Float).SetInt(new(big.Int).Exp(big.NewInt(10), big.NewInt(int64(decimals)), nil))

	// Chuyển đổi value sang big.Float
	val := new(big.Float).SetInt(value)

	// Chia value / factor để lấy giá trị chuẩn
	result := new(big.Float).Quo(val, factor)

	// Trả về kết quả dưới dạng string với tối đa 6 chữ số sau dấu thập phân
	return result.Text('f', round) 
}
func FloatRound(num float64, places int) string {
	return fmt.Sprintf("%.*f", places, num)
}

// Random returns a random integer between min and max (inclusive)
func Random(min, max int) int {
	// Create a new random number generator using crypto/rand
	nBig, err := rand.Int(rand.Reader, big.NewInt(int64(max-min+1)))
	if err != nil {
		// Fallback to a simple pseudo-random number if crypto/rand fails
