...
func GetConfig(vault *vault.Vault, config_path string) map[string]string {
	...
	m["CHECK_DB"] = vault.ReadVAR(fmt.Sprintf("%s/CHECK_DB", config_path))
	return m
}
func MergeConfig(global, local map[string]string) map[string]string {
	...
	if utils.Map_contains(global, "CHECK_DB") || utils.Map_contains(local, "CHECK_DB") {
		if utils.Map_contains(local, "CHECK_DB") && local["CHECK_DB"] != "" {
			m["CHECK_DB"] = local["CHECK_DB"]
		} else {
			m["CHECK_DB"] = global["CHECK_DB"]
		}
	}
	return m
}
